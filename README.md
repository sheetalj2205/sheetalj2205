<h1 align="center">Hi 👋, I'm Sheetal Jain</h1>
<h3 align="center">Master’s Student at SUNY Buffalo | Software Engineer | Backend & Scalable Systems Enthusiast</h3>

<!-- <p align="left"> <img src="https://komarev.com/ghpvc/?username=sheetalj2205&label=Profile%20views&color=0e75b6&style=flat" alt="sheetalj2205" /> </p> -->

- 🎓 I’m currently pursuing my **Master’s in Computer Science at SUNY Buffalo**, expanding my expertise across core computer science domains with a focus on scalable systems, secure architectures, and advanced computing technologies.

- 🌱 I’m currently learning **Data Structures & Algorithms, System Design and AI technologies** to strengthen my problem-solving skills and build intelligent, scalable systems.

- 💼 Previously worked as a **Software Engineer in FinTech**, building and scaling Java-based backend systems supporting global users. I developed reliable services, modernized legacy systems, strengthened authentication and security, integrated third-party platforms, and improved system performance and deployment workflows.

- 👩‍💻 Interned at **AutoWiz and HotWax**, working with **GCP, BigQuery, Looker, Python, and Moqui**, gaining hands-on experience in cloud platforms, data systems, and production environments.

- 🎓 Completed my **Bachelor of Technology in Computer Science and Engineering** from Ujjain Engineering College (June 2023).

- ✍️ I actively share insights on **technology, career growth, and personal branding** on LinkedIn and Twitter. I enjoy building in public and contributing to the tech community.

- 📫 How to reach me **sheetalj2205@gmail.com**

<h3 align="left">Connect with me:</h3>
<p align="left">
<a href="https://twitter.com/sheetal2205" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/twitter.svg" alt="sheetal2205" height="30" width="40" /></a>
<a href="https://linkedin.com/in/sheetaljain225" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="sheetaljain225" height="30" width="40" /></a>
<a href="https://medium.com/@sheetalj2205" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/medium.svg" alt="@sheetalj2205" height="30" width="40" /></a>
</p>

<h3 align="left">Technical Skills:</h3>

- 💻 **Programming Languages:** Java, C++, Python, Scala, SQL  
- 🌐 **Web Technologies:** HTML, CSS, JavaScript, XML, Servlets, JSP  
- ⚙️ **Backend Frameworks:** Spring Boot, Dropwizard, Hibernate, Moqui  
- 🧪 **Testing:** JUnit, Mockito  
- 📊 **Data & Analytics:** BigQuery, Snowflake, Looker  
- 🔄 **Messaging & Streaming:** Apache Kafka  
- ☁️ **Cloud Platforms:** GCP, AWS  
- 🐳 **DevOps & Containerization:** Docker, Kubernetes, Jenkins  
- 📈 **Monitoring & Incident Management:** Sumo Logic, PagerDuty  
- 🛠 **IDEs:** VS Code, Eclipse, NetBeans, IntelliJ IDEA
- 🤖 **Generative AI & LLMs:** LLM APIs, Retrieval-Augmented Generation (RAG), Langchain

### 📝 My Latest Blog Post in Medium
<a target="_blank" href="https://github-readme-medium-recent-article.vercel.app/medium/@sheetalj2205/0"><img src="https://github-readme-medium-recent-article.vercel.app/medium/@sheetalj2205/0" alt="Recent Article 0">
<a target="_blank" href="https://github-readme-medium-recent-article.vercel.app/medium/@sheetalj2205/1"><img src="https://github-readme-medium-recent-article.vercel.app/medium/@sheetalj2205/1" alt="Recent Article 1">
